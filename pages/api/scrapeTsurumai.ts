  ...
  try {
    ...
    const newData = await page.evaluate((selectors) => {
      ...
      if (descElement) {
        ...
        console.log(`Description found: ${data.description.substring(0, 50)}...`);
      } else {
    ...
    if (error instanceof Error) {
      throw new AppError(`Error scraping Tsurumai: ${error.message}`, 500, { singleProductJan, rule });
    }
    throw new AppError('Unknown error scraping Tsurumai', 500, { singleProductJan, rule });
  } finally {
    await page.close();
  }
}
