...
export async function scrapeAmiami(browser: Browser, singleProductJan: string, rule: any) {
  ...
  try {
    ...
    const productBoxes = await page.$$('.product_table_list .product_box, .product_box');
    console.log(`Found ${productBoxes.length} product boxes`);

    if (productBoxes.length > 0) {
      const lastProductBox = productBoxes[productBoxes.length - 1];
      const productLink = await lastProductBox.$eval('a', (el) => el.href).catch(() => null);
      ...
      if (productLink) {
        ...
        const result = await page.evaluate(() => {
          ...
          if (explainDiv) {
            ...
            const descHeading = Array.from(explainDiv.querySelectorAll('p.heading_07')).find(el => el.textContent?.includes('解説'));
            if (descHeading) {
    ...
    if (error instanceof Error) {
      throw new AppError(`Error scraping Amiami: ${error.message}`, 500, { singleProductJan, rule });
    }
    throw new AppError('Unknown error scraping Amiami', 500, { singleProductJan, rule });
  } finally {
    await page.close();
  }
}
